...
namespace PictureMemoryTraining.Views.Models
{
    public static class MemoryPictureItemsManager
    {
        ...
        private static List<List<MemoryPictureItem>> _allFamiliarMemoryPictures = null;
        private static List<List<MemoryPictureItem>> GetFamiliarMemoryPictures()
        {
            if (_allFamiliarMemoryPictures == null)
            {
                var memoryPictures = MemoryPictureManager.GetMemoryPictureGroups();
                _allFamiliarMemoryPictures = memoryPictures.Select(i => i.Select(j => new MemoryPictureItem()
                {
                    ImageUri = j
                }).ToList()).ToList();
            }

            return _allFamiliarMemoryPictures;
        }
